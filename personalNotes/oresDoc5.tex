...
%
%
\section{Revision data, RecentChanges}
...
\begin{description}
\item \includegraphics[scale=0.3]{resources/4/PRQT}
\item Reads data of the following format (txt file):
\item ''State,Account Length,Area Code,Phone,...\\
KS,128,415,382-4657,...\\
OH,107,415,371-7191,...''
\end{description}
\item Discrimination Threshold Visualization (\href{http://www.scikit-yb.org/en/latest/api/classifier/threshold.html}{Link}):
\begin{description}
\item \includegraphics[scale=0.3]{resources/4/discriminationThresholdVisualization}
\item Reads .csv files
\end{description}
\end{itemize}
...
\begin{description}
\item \underline{Interesting}: ``providing a visualisation of both the most-confident positive and most-confident negative samples led to higher quality and faster concept learning than providing the full ranking of images''
\end{description}
...
\begin{description}
\item \includegraphics[scale=0.45]{resources/5/fourInterfaceElements}
\item ``We do not suggest that these elements are necessarily visually distinct, or indeed necessary in all applications of IML.''
\item ``one-time only custom task may have limited or no use for the model inspection or task overview interfaces''
\end{description}
\end{itemize}
...
\end{itemize}
\begin{description}
\item User could base his decision on contextualisations like those
\end{description}
\end{itemize}
...
\end{enumerate}
\begin{description}
\item Other mentioned risks:
\begin{itemize}
\item Frustration due to lack of knowledge e.g. how to eradicate false positives/negatives (the model will probably need to undergo quite a few steps of change before the wanted result is produced)
\item Overfitting
\end{itemize}
\end{description}
\end{itemize}
...
\begin{description}
\item Maximise user perception to features relevant to understanding functions and deficiencies of the model
\item Interfaces supporting complex data interaction: aim at simplification of data (clusters etc.) and ``amplify user's cognitive ability'' (highlight regions etc.)
\end{description}
\end{itemize}
...
\begin{description}
\item ``the IML interface should be maximally interactive and should exploit rich forms of interaction where possible, for example direct sketching on images, ...''
\end{description}
